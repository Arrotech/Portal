...
@portal_v1.route('/registrar/register', methods=['POST', 'GET'])
def registrar_signup():
    """A new registrar can create a new account."""
    ...
    response = UsersModel(firstname, lastname, surname,
                            admission_no, gender, email, password).save_registrar()
...
@portal_v1.route('/staff/role/<int:user_id>', methods=['PUT'])
@jwt_required
@registrar_required
def promote_admin(user_id):
    ...
    details = request.get_json()
    role = details['role']
    response = UsersModel().update_admin_role(role, user_id)
    if response:
        return Serializer.serialize(response, 200, "Admin promoted successfully")
    return Serializer.serialize(response, 404, "User not found")
