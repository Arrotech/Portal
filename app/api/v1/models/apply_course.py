...
class ApplyCoursesModel(Database):
    ...
    def save(self):
        """Apply for a course."""
        self.curr.execute(
            ''' INSERT INTO apply_course(student, institution, campus,\
            certificate, department, course, created_on)
            VALUES('{}','{}','{}','{}','{}','{}','{}')
            RETURNING student, institution, campus, certificate, department,\
            course, created_on'''
            .format(self.admission_no, self.institution_name, self.campus_id,
                    self.certificate_id, self.department_name,
                    self.course_name, self.created_on))
    ...
    def get_course_info_by_admission_no(self, admission_no):
        ...
        query = "SELECT u.firstname, u.lastname, u.surname, u.admission_no,\
                i.institution_name,d.department_name, c.course_name,\
                ca.campus_name, ce.certificate_name FROM apply_course AS a\
                INNER JOIN users AS u ON a.student=u.admission_no\
                INNER JOIN institutions AS i on\
                a.institution=i.institution_name\
                INNER JOIN campuses AS ca ON a.campus=ca.campus_id\
                INNER JOIN certificates AS ce ON\
                a.certificate=ce.certificate_id\
    ...
    def update(self, application_id, institution_name, campus_id,
        ...
        self.curr.execute(
            """UPDATE apply_course SET institution='{}', campus='{}',\
            certificate='{}',department='{}',course='{}'\
            WHERE application_id={} RETURNING institution, campus,\
            certificate, department, course""".format(application_id,
                                                      institution_name,
                                                      campus_id,
                                                      certificate_id,
                                                      department_name,
    ...
    def delete(self, application_id):
        ...
        self.curr.execute(
            """DELETE FROM apply_course WHERE application_id={}"""
